  ...
  return (
    <FormProvider {...methods}>
      <form
        ...
        className="w-full max-w-[700px] rounded-2xl bg-white p-7 text-[#2b4246]"
      >
        <h1 className="mb-7 text-2xl font-bold text-[#2b4246]">Contact US</h1>
        <fieldset className="md: mb-4">
          <div className="flex gap-x-5 space-y-3">
            <InputField
              label="First Name"
              name="firstName"
              className="w-full"
        ...
        <fieldset className="my-6">
          <label className="flex cursor-pointer items-center">
            ...
            {/* CHECKBOX STYLIZED */}
            <div
              className={`${methods.watch("terms") && "absolute border-transparent"} mr-2 h-[18px] w-[18px] border border-[#87a3a6]`}
            />
            <div
              className={`${methods.watch("terms") ? "mr-2 block" : "hidden"}`}
            >
              <img src={iconCheck} alt="Checkbox" />
            </div>
